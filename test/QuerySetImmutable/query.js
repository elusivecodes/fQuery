...
describe('#query', function() {
    ...
    it('finds elements by query selector', async function() {
        assert.deepEqual(
            await exec(_ =>
                dom.query('#parent1 > #child1 > span, #parent1 > #child2 > span')
                    .get()
                    .map(node => node.id)
            ),
            [
    ...
    it('finds elements by custom child selector', async function() {
        assert.deepEqual(
            await exec(_ =>
                dom.query('> .group1 > .group1, > .group2 > .group2', '#child1, #child4')
                    .get()
                    .map(node => node.id)
            ),
            [
    ...
    it('returns a QuerySetImmutable', async function() {
        assert.equal(
            await exec(_ =>
                dom.query('div') instanceof QuerySetImmutable
            ),
            true
        );
    });
