...
test.describe('QuerySet #append', () => {
    ...
    test('works with Document nodes', async ({ page }) => {
        const childCount = await page.evaluate(() => {
            ...
            $(myDoc).append(myDoc.createElement('html'));
    ...
    test('works with HTMLElement other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('div').append(document.querySelector('.test1'));
    ...
    test('works with NodeList other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('div').append(document.querySelectorAll('a'));
    ...
    test('works with HTMLCollection other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('div').append(document.getElementById('parent1').children);
    ...
    test('works with DocumentFragment other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('div').append(document.createRange().createContextualFragment('<div><span></span></div>'));
