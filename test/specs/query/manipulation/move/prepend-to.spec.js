...
test.describe('QuerySet #prependTo', () => {
    test.beforeEach(async ({ page }) => {
    ...
    test('works with DocumentFragment nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $(document.createRange().createContextualFragment('<div><span></span></div>')).prependTo('div');
    ...
    test('works with HTMLElement other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('a').prependTo(document.getElementById('parent1'));
    ...
    test('works with NodeList other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('a').prependTo(document.querySelectorAll('div'));
    ...
    test('works with HTMLCollection other nodes', async ({ page }) => {
        const result = await page.evaluate(() => {
            $('a').prependTo(document.body.children);
    ...
    test('works with Document other nodes', async ({ page }) => {
        const childCount = await page.evaluate(() => {
            ...
            $(myDoc.createElement('html')).prependTo(myDoc);
